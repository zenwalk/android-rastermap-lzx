...
import com.example.androidrastermap.document.multitouch.MultiTouchZoom;
...
public class DocumentView extends View implements ZoomListener {
    ...
    private void goToPageImpl(final int toPage) {
    	int a=pages.get(toPage).getLeft();
        scrollTo(a,0);
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
            	float nowX=ev.getX();
            	float nowY=ev.getY();
                scrollBy((int) (lastX - nowX), (int) (lastY - nowY));
                setLastPosition(ev);
                break;
    ...
    void invalidatePageSizes() {
      ...
      //  float heightAccum = 0;
        ...
        for (int i = 0; i < pages.size(); i++) {
           ...
           //Ë³ÐòÎª×ó¾à ÉÏ¾à ÓÒ¾à ÏÂ¾à
            //page.setBounds(new RectF(0, heightAccum, width * zoom, heightAccum + pageHeight));
            //heightAccum += pageHeight;
            //ºáÆÁÉèÖÃ
            page.setBounds(new RectF(widthf,0,widthf+width,pageHeight));
            widthf+=width;
        }
    }
